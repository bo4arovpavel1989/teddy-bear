					...
					try {
						if (rep != null && rep.passwd === passwd) {
							generateSSID(login, function(SSID){
								var session = new Session({login: 'admin', session: SSID}).save();
...
module.exports.orderToWork = function(req, res){
	...
	Admin.update({login: 'admin'}, {$inc: {newOrders: -1}}).exec();
	Order.findOne({_id: orderId}, 'email', function(err, rep){
					if(rep) sendEmail(rep.email, 'Ваш заказ', 'Ваш заказ принят в работу!', ''); 
				});
	res.end();	
};
