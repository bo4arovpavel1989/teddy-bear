...
module.exports.makeOrder = function(req, res) {
	...
	data.date = Date.now() + 10800000; //adjusting timezone
	...
	for (var i=0; i < productCount; i++){
		...
		if (productId) Product.findOne({_id: productId}, 'price isSpecialProp discount name', function(err, rep){
			if(rep) {
				if (rep.isSpecialProp) 	data.product.forEach(function(productItem){
											if(productItem.name == rep.name) productItem.price = rep.discount;
										});
				else data.product.forEach(function(productItem){
						if(productItem.name == rep.name) productItem.price = rep.price;
					 });
			}
			step++;
