...
module.exports.addSpecialPropForProduct = function(req, res){
	...
	try {
		Product.findOne({_id: req.body['product']}, 'price address', function(err, rep){
			if(rep) {
				if (req.body['discount'] !== '') var newPrice = rep.price - (0.01 * rep.price * Number(req.body['discount'])); /*calculates new price by discount*/
				else var newPrice = req.body['new_price'];
