	...
	try {
		...
		form.parse(req, function(err, fields, files) {
			...
			Category.findOne({_id: fields.category}, function(err, rep){
				if(rep) {
					var category={name: rep.name, id: rep._id.toString()}; /*variable to save in product schema*/
					var subcategory;
	...
	try {
		Product.findOne({_id: req.body['product']}, 'price address', function(err, rep){
			if(rep) {
				...
				var specProp = new SpecialProp({describe: req.body['describe'], target: target, durancy: req.body['toDate'], link: rep.address, discount: req.body['discount']}).save();
			}
		});
	} catch(e){}
	res.redirect('/admin');
...
module.exports.addSpecialPropForCategory = function(req, res){
	...
	try {
		Product.find({'category.id': target.id}, 'price address', function(err, reps){
			if(reps) {
				console.log('Propduct for specail prop');
				console.log(reps);
				reps.forEach(function(rep){
					var newPrice = rep.price - (0.01 * rep.price * Number(req.body['discount']));
					Product.update({_id: rep._id}, {$set: {isSpecialProp: true, discount: newPrice}}).exec();
				});	
			var specProp = new SpecialProp({describe: req.body['describe'], target: target, durancy: req.body['toDate'], link: req.body['category'], discount: req.body['discount']}).save();
			}
		});
	} catch(e) {}
	res.redirect('/admin');
