	...
	try {
		...
		form.parse(req, function(err, fields, files) {
			...
			if (files.upload.size !== 0 && files.upload.size !== undefined) {
				...
				Category.update({_id: categoryId}, {$set: {name: newName, image_micro_thumb: htmlMicroThumbFileName}}).exec(function(err, rep){
					res.end();
				});
			}	else Category.update({_id: categoryId}, {$set: {name: newName}}).exec(function(err, rep){
						res.end();
					});
		});
	} catch(e) {}
		...
		form.parse(req, function(err, fields, files) {
			...
			Category.findOne({_id: fields.category}, function(err, rep){
				if(rep) {
					...
					subcategory: subcategory, address: fields.address, price: fields.price, quantity: Number(fields.quantity), rank: Number(fields.rank),
		...
		form.parse(req, function(err, fields, files) {
		    ...
		    if (files.upload.size !== 0 && files.upload.size !== undefined) { /*this is the case, where user uploads photo*/
				...
				Product.update({_id: fields.id}, {$set: {name: fields.name,  nameUpperCase: fields.name.toUpperCase(), 
								address: fields.address, price: fields.price, quantity: Number(fields.quantity), rank:  Number(fields.rank),
