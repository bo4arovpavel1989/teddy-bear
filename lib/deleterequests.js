...
module.exports.deleteCallback = function(req, res){
	...
	if (callbackId) Callback.findOne({_id: callbackId}, 'isItNew newCalls', function(err, rep){
						if(rep.isItNew && rep.newCalls>0) Admin.update({login: 'admin'}, {$inc: {newCalls: -1}}).exec();
...
module.exports.deleteOrder = function(req, res){
	...
	if (orderId) Order.findOne({_id: orderId},  'isItNew newOrders', function(err, rep){
					if(rep.isItNew && rep.newOrders>0) Admin.update({login: 'admin'}, {$inc: {newOrders: -1}}).exec();
