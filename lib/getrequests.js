...
module.exports.getProductsList = function(req, res){
	...
	if (counter!==undefined) { /*check if filter for categories was used*/
		...
		categories.forEach(function(category){
			Product.find({'category.id': category}, 'name address quantity isHit image_micro_thumb', function(err, rep){
		...
		Product.find({isHit: true}, 'name address quantity isHit image_micro_thumb', function(err, rep){
...
module.exports.getFeedback = function(req, res){
	...
	console.log(data);
	Feedback.find({}).sort({_id: -1}).limit(10).exec(function(err, rep){
