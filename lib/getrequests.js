...
module.exports.getOrders = function(req, res){
	...
	Order.find({$or: [{isItNew: true}, {isActive: true}]}, function(err, rep){
		if(rep) data.order = rep.reverse();
...
module.exports.getOrderArchive = function(req, res){
	...
	Order.find({isItNew: false, isActive: false, deletedOne: false}, function(err, rep){
		if(rep) data.order = rep.reverse();
		res.render('components/orders', data);
	});
};
...
module.exports.getOrderDeleted = function(req, res){
	...
	Order.find({isItNew: false, isActive: false, deletedOne: true}, function(err, rep){
		if(rep) data.order = rep.reverse();
		res.render('components/orders', data);
	});
}
...
module.exports.searchClient = function(req, res){
	...
	Order.find({name: {$regex: reg}}, function(err, rep){
		if(rep!== null && rep!== undefined && rep.length !== 0) {
			data.order = rep.reverse();
			res.render('components/orders', data);
		} else {
